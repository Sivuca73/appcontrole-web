...
export function MeetingMidweek({ meioSemana, mecanicas }: MeetingMidweekProps) {
  // Se não houver dados da reunião, mostra aviso amigável sem quebrar
  if (!meioSemana) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center bg-white rounded-2xl border border-gray-100 min-h-[300px] shadow-2xs">
        <Calendar className="w-12 h-12 text-[#34727D]/40 mb-4" />
  ...
  // Filtragem mecânica segura para manter apenas Mídias no rodapé
  ...
  return (
    <div className="space-y-8" id="meio-semana-programacao-view">
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-5 bg-linear-to-br from-slate-50 to-white hover:shadow-xs transition duration-300">
      ...
      {mecanicas && <MechanicalSupportGrid designacoes={filtradasMecanicas} />}
    </div>
  );
}
